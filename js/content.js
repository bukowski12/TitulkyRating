function displayRatings(imdbNr, fallbackTitle, fallbackYear) {
  ...
  $.getJSON("https://www.omdbapi.com/?i=" + imdbNr + "&apikey=2c7f8b02", function (JSONdata) {
    var movieTitle = JSONdata.Title || fallbackTitle;
    var movieYear = (JSONdata.Year ? JSONdata.Year.substr(0, 4) : fallbackYear);
    var ratingBg = "plus-rating-none";
    var url;

    var ratingVal = "N/A";
    url = $("a[target='imdb']").attr("href") || ("https://www.imdb.com/title/" + imdbNr);
    ...
    if (JSONdata.imdbRating && $.isNumeric(JSONdata.imdbRating)) {
      ...
      ratingVal = JSONdata.imdbRating;
      $("a[target='imdb']").find("img").hide();
    }

    $(".plus-rating-container").append('<div title="Hodnocení na IMDB" class="plus-rating plus-rating-imdb ' + ratingBg + '"><a href="' + url.replace(/<[^>]*>?/g, "") + '" target="_blank">' + ratingVal.replace(/<[^>]*>?/g, "") + "</a></div>");
    ...
    var metaVal = "N/A";
    url = "https://www.metacritic.com/search/movie/" + encodeURIComponent(movieTitle) + "/results";
    if (JSONdata.Type == "series") {
      url = "https://www.metacritic.com/search/tv/" + encodeURIComponent(movieTitle) + "/results";
    }
    ...
    if (JSONdata.Metascore && $.isNumeric(JSONdata.Metascore)) {
      ...
      metaVal = JSONdata.Metascore;
    }

    $(".plus-rating-container").append('<div title="Hodnocení na MetaCritic" class="plus-rating plus-rating-metacritic ' + ratingBg + '"><a href="' + url.replace(/<[^>]*>?/g, "") + '" target="_blank">' + metaVal.replace(/<[^>]*>?/g, "") + "</a></div>");
    ...
    if (JSONdata.Response !== "False" && JSONdata.Title && JSONdata.Year) {
      url = "https://www.csfd.cz/hledat/?q=" + encodeURIComponent(movieTitle) + "+" + movieYear;

      chrome.runtime.sendMessage(url, function (csfdFindRAW) {
        var imdbmovietitle = JSONdata.Title;
        var movietype = ".main-" + (JSONdata.Type ? JSONdata.Type.substring(0, 5) : "movie") + "s .article-content";
        ...
        if ($(csfdFindRAWdata).find(movietype).length) {
          $(csfdFindRAWdata).find(movietype).each(function (index, value) {
            ...
            if (normImdb == normCsfd || normImdb == normCsfdOther || normImdbAlt == normCsfd || normImdbAlt == normCsfdOther) {
              ...
              $(".plus-rating-container").append('<div title="Hodnocení na CSFD" class="plus-rating plus-rating-csfd plus-rating-none"><a href="' + url.replace(/<[^>]*>?/g, "") + '" target="_blank">N/A</a></div>');
              $(".plus-rating-container").append('<div title="CSFD" class="plus-rating-img plus-rating-csfd plus-rating-none"><a href="' + url.replace(/<[^>]*>?/g, "") + '" target="_blank"><img src="' + (-1 !== navigator.userAgent.indexOf("Chrome") ? chrome.runtime : browser.extension).getURL("images/imgcsfd.png") + '" alt="CSFD"></a></div>');
      ...
      // OMDb API error or missing Title/Year
      var searchUrl = "https://www.csfd.cz/hledat/?q=" + encodeURIComponent(movieTitle) + "+" + movieYear;
      $(".plus-rating-container").append('<div title="Hodnocení na CSFD" class="plus-rating plus-rating-csfd plus-rating-none"><a href="' + searchUrl.replace(/<[^>]*>?/g, "") + '" target="_blank">N/A</a></div>');
      $(".plus-rating-container").append('<div title="CSFD" class="plus-rating-img plus-rating-csfd plus-rating-none"><a href="' + searchUrl.replace(/<[^>]*>?/g, "") + '" target="_blank"><img src="' + (-1 !== navigator.userAgent.indexOf("Chrome") ? chrome.runtime : browser.extension).getURL("images/imgcsfd.png") + '" alt="CSFD"></a></div>');
      $(".plus-rating-img.plus-rating-csfd").append('<div class="plus-badge-black" title="Snímek nenalezen &#10;- Neshoduje se název nebo rok výroby"></div>');
    }
  });
}
$(document).ready(function () {
  var search, title, CookieDate = new Date, imdb = (CookieDate.setFullYear(CookieDate.getFullYear() + 10),
    ...
    $("h1").length && $("a[target='imdb']").length && -1 == location.href.indexOf("pozadavek-") && (function () {
      var h1Text = $("h1").text();
      var titleParts = h1Text.split(" (");
      title = titleParts[0].trim();
      var extractedYear = titleParts[1] ? titleParts[1].substring(0, 4) : "";
      imdb = $("a[target='imdb']").attr("href").split("title/")[1];
      displayRatings(imdb, title, extractedYear);
    })()
  );
});